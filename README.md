Running miniGhost with CAB-MPI
...
### Step 1: Build cab_essential (on remote machine)
```bash
...
### Step 2: Build cab_glibc (on remote machine)
```bash
cd $cab_glibc
...
### Step 3: Build cab_pip (on remote machine)
```bash
cd $cab_pip
...
### Step 4: Build mpich (on remote machine)
Select one of the mpich to build
```bash
...
### Step 5: Run miniGhost (on remote machine)
```bash
cd $miniGhost
module purge
cd ref
...
Example output of running miniGhost:
...
The pip-coll-double-copy can run correctly on the localhost, but it doesn't contains stealing code
```bash
# Also, pip-coll-double-copy run on another node will failed too.
# therefore, it can't run when num_node >= 2
